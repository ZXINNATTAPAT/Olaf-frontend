...
test('renders CommentSection with initial comments', () => {
  const comments = [
    { comment_id: 1, comment_text: 'Comment 1', comment_datetime: '2024-01-01', like_count: 0 }
  ];
  render(<CommentSection post_id={1} initialComments={comments} />);
  expect(screen.getByText('Comment 1')).toBeInTheDocument();
});

test('renders CommentSection without comments', () => {
  render(<CommentSection post_id={1} />);
