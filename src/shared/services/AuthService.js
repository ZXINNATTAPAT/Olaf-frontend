    ...
    async login(email, password) {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                console.log('🍪 Login Response - Headers (Set-Cookie is forbidden, check Network tab):', {
            ...
            if (process.env.NODE_ENV === 'development') {
                setTimeout(() => {
                    ...
                    console.log('🍪 Has auth cookies (from document.cookie):', hasAuthCookies);
                    if (!hasAuthCookies) {
                        console.warn('⚠️ HttpOnly cookies may not appear in document.cookie. Check Application tab → Cookies instead.');
                        console.warn('⚠️ If cookies are missing, check backend CORS and Set-Cookie configuration.');
                    }
                }, 200);
            }
